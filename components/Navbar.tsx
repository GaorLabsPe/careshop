...
const Navbar: React.FC<NavbarProps> = ({ onCartToggle, onNavigate }) => {
  ...
  return (
    <nav className="sticky top-0 z-50 bg-white border-b border-slate-100 shadow-sm">
      <div className="bg-[#064E3B] text-white text-[10px] py-2 px-6 hidden sm:flex justify-between items-center font-ubuntu uppercase tracking-[0.2em]">
        <div className="flex items-center gap-8">
          <span className="flex items-center gap-2 opacity-90"><ShieldCheck size={12} className="text-[#10B981]" /> Resolución Sanitaria Vigente</span>
          <span className="flex items-center gap-2 opacity-90"><Phone size={12} className="text-[#F97316]" /> Central: 01 444 5555</span>
        </div>
      ...
      <div className="max-w-[1440px] mx-auto px-4 sm:px-10 h-20 sm:h-24 flex items-center gap-4 sm:gap-10">
        ...
        >
          <div className="w-10 h-10 sm:w-12 sm:h-12 bg-[#10B981] text-white rounded-xl sm:rounded-2xl flex items-center justify-center shadow-lg group-hover:bg-[#059669] transition-all group-hover:scale-105">
            <ShoppingBag size={24} />
          </div>
        ...
        <div className="flex-1 relative group max-w-2xl">
          ...
          <input 
            ...
            className="w-full bg-slate-50 border-2 border-slate-100 rounded-xl sm:rounded-[1.5rem] py-2.5 sm:py-3.5 pl-4 sm:pl-12 pr-4 sm:pr-24 focus:bg-white focus:border-[#10B981] outline-none transition-all text-xs sm:text-sm font-bold text-slate-800 placeholder:text-slate-400"
          />
          <button className="absolute right-2 top-1/2 -translate-y-1/2 bg-[#10B981] text-white px-4 sm:px-6 py-1.5 sm:py-2 rounded-lg sm:rounded-xl font-bold text-[9px] uppercase tracking-widest hover:bg-[#059669] transition-colors hidden xs:block font-ubuntu">
            Buscar
          </button>
        </div>
